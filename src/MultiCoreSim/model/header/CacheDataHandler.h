...
namespace ns3
{
    ...
    protected:
        ...
        {
            return (address >> ((int)log2(m_block_size) + (int)log2(m_banks_count)  + (int)log2(m_sets_count)));
        }

        inline uint64_t calculate_set(uint64_t address)
        {
            return ((address >>((int)log2(m_block_size)+ (int)log2(m_banks_count) )) & (m_sets_count - 1));
        }
        inline uint64_t calculate_bank(uint64_t address)
        {
            return ((address >>((int)log2(m_block_size) )) & (m_banks_count - 1));
        }

        inline uint64_t calculate_address(uint64_t tag, uint64_t set)
        {
            return (tag << ((int)log2(m_block_size) + (int)log2(m_banks_count) + (int)log2(m_sets_count))) |
                   (set << ((int)log2(m_block_size)+ (int)log2(m_banks_count)))|
                   (m_bankNumber << ((int)log2(m_block_size)));
        }
